...
def _test_lbl(robot):
    robot.setLBL()
    robot.setLBL()
    robot.setLBL(labelName='Three')
    robot.SETCOUNTER = 1000
    robot.setLBL('SETCOUNTER', 'set counter')
    robot.setLBL('SETCOUNTER', 'set counter2')
    #robot.DOUBLECOUNTER = 1000
    #robot.setLBL('DOUBLECOUNTER', 'error')
    robot.jump2LBL(1)
    robot.POCKETLOOP = 5000
    robot.setLBL('POCKETLOOP', 'pocket_loop')
    robot.jump2LBL(5000)
    robot.jump2LBL(numReg=215)
    robot.ifOnJump('R[1:i]>=R[4:inc]', 5001)
    robot.ifOnJump('R[2:j]>=0', numReg=215)
...
def test_post():
    ...
    robot.MoveL(Pose([200, 250, 348.734575, 180, 0, -150]), [-41.62707, -8.89064, -
                                                             30.01809, 60.62329, 49.66749, -258.98418])
    # time after digital output turn on
    ...
    robot.TIMEAFTER = (0, "G0_LASER_STOP")
    # turn off offset
    del robot.P_OFFSET
    ...
    robot = _test_lbl(robot)

    robot.waitMS(50)
    robot.waitDI(50, 1, 500)
